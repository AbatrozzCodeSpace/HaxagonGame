...
public class PruningParkMinimaxPlayer implements Player {
    String me;
    int maxdepth;
	int secureScore;
    ...
    public PruningParkMinimaxPlayer(int md, int secureScore, int noneSecureNullState, int noneSecureJumpState, int noneSecureWalkState) {
		maxdepth = md;
     ...
     */
    public int evalMove(Move m, State s, int depth, int alpha, int beta) {
		int a = alpha;
		int b = beta;
		count++;
	...
	if (depth == 0) { // cutting off search -> evaluate resulting state
	    ...
	    int minimaxvalue = evalMove((Move) it.next(), newstate, depth - 1, a, b);
	    // now the rest of the moves
	    while (it.hasNext()) {
		Move newmove = (Move) it.next();
		int eval = evalMove(newmove, newstate, depth - 1, a, b);
		...
		if (newstate.whoseTurn().equals(me)){ // our turn
			...
			if(minimaxvalue > a) a= minimaxvalue;
			if(alpha >= b) break;
		}
		else{ // opponent's turn
		    minimaxvalue = Math.min(minimaxvalue, eval);
			if(minimaxvalue < b) b= minimaxvalue;
			if(alpha >= b) break;
		}
	    }
	...
	int minimaxvalue = evalMove(bestMove, s, maxdepth, -9999999, 9999999);

	// now the rest of the moves
	while (it.hasNext()) {
	    Move move = (Move) it.next();
	    int eval = evalMove(move, s, maxdepth, -9999999, 9999999);
